    ...
    if file_uploaded is not None:
        ...
        with col2:
            ...
            if st.button("Apply Negative"):
                ...
                display_image(col3, imgout, caption="Negative Image")

            if st.button("Apply Logarithmic"):
                imgout = c3.Logarit(imgin)
                display_image(col3, imgout, caption="Logarithmic Image")

            if st.button("Apply Power"):
                imgout = c3.Power(imgin)
                display_image(col3, imgout, caption="Power Image")

            if st.button("Apply PiecewiseLinear"):
                imgout = c3.PiecewiseLinear(imgin)
                display_image(col3, imgout, "PiecewiseLinear Image")
